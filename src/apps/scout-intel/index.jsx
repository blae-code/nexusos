...
import SignatureCalculator from './signature/SignatureCalculator';
...
export default function ScoutIntel() {
  ...
  const validTabs = ['deposits', 'routes', 'livemap', 'signature'];
  ...
  const SCOUT_TABS = [
    ...
    { id: 'signature', label: 'SIGNATURE CALC' },
  ];

  if (scoutTab === 'signature') {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
        <div style={{ display: 'flex', gap: 0, padding: '0 16px', borderBottom: '0.5px solid rgba(200,170,100,0.10)', background: '#0A0908', flexShrink: 0 }}>
          {SCOUT_TABS.map(t => (
