...
export default function FleetImportDialog({ onClose, onImported, ownerCallsign }) {
  ...
  const handleImport = async () => {
    ...
    // Batch create — strip internal fields
    const records = toImport.map(s => {
      const { _source, _entity_type, ...ship } = s;
      // Clean undefined values
      return Object.fromEntries(Object.entries(ship).filter(([, v]) => v !== undefined && v !== ''));
    });
    ...
    setStep('done');
    showToast(`Imported ${success} ship${success !== 1 ? 's' : ''} to fleet roster`, 'success');
  };
