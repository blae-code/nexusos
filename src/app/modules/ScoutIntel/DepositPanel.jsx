...
function TopDepositRow({ deposit, onClick }) {
...
function GapRow({ name, have, need, bestDeposit }) {
...
function LeaderRow({ rank: pos, callsign, count, avgQuality }) {
  return (
    <div style={{
      display: 'flex', alignItems: 'center', gap: 8,
...
function DefaultMode({ deposits, materials, blueprints, onSelectDeposit }) {
...
function DetailMode({ deposit, liveOp, callsign, onBack, onDepositUpdated }) {
  ...
  const handlePlanRun = async () => {
    ...
    try {
      ...
      console.error('[DepositPanel] plan run failed:', e);
