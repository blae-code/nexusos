...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!user) return Response.json({ error: 'invalid_key' }, { status: 401, headers: sessionNoStoreHeaders() });
    if (user.key_revoked) return Response.json({ error: 'key_revoked' }, { status: 403, headers: sessionNoStoreHeaders() });
    if (!user.auth_key_hash) return Response.json({ error: 'invalid_key' }, { status: 401, headers: sessionNoStoreHeaders() });
    ...
    await base44.asServiceRole.entities.NexusUser.update(user.id, {
      ...
      is_admin: isAdmin, key_revoked: false,
