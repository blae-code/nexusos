/**
 * uexStantonCrawler — Canonical automated commodity sync entrypoint.
 */
...
Deno.serve(async (req) => {
  try {
    ...
    const triggerMode = body.trigger_mode === 'manual' ? 'manual' : 'auto';
    const result = await runCommodityMarketSync(base44, {
      triggerMode,
      triggeredBy: triggerMode === 'auto'
        ? 'AUTO'
        : (typeof body.triggered_by === 'string' ? body.triggered_by : 'MANUAL'),
    });

    return Response.json(result, { status: result.status === 'failed' ? 500 : 200, headers: NO_STORE });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.error('[uexStantonCrawler]', message);
