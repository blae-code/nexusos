...
- `NexusUser` is the only member-auth source of truth for issued-key flows.
- Base44 native `User` auth remains reserved for the platform administrator path only.
...
- `docs/auth-schema.md` defines the live `NexusUser` auth contract and required persistence checks.

## Runtime Model

- `src/core/data/base44Client.js` uses the same-origin Vite proxy on localhost and the deployed Base44 host elsewhere.
- `src/core/data/auth-api.js` always talks to same-origin `/api/functions/*`.
