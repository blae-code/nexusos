...
function generateAuthKey() {
  // KEY_CHARSET is exactly 32 chars; 32 divides 256 evenly — no modulo bias.
  const seg = () => {
    const bytes = crypto.getRandomValues(new Uint8Array(4));
    return Array.from(bytes, b => KEY_CHARSET[b % 32]).join('');
  };
...
async function assertCaller(base44) {
  ...
  // key_issued_by is set to 'SYSTEM_ADMIN' sentinel so audit trail is never null.
  if (user.role === 'admin') {
    return { nexus_rank: 'PIONEER', discord_id: 'SYSTEM_ADMIN', callsign: 'ADMIN' };
  }
...
async function handleGenerate(base44, caller, body) {
  ...
  // Field validation.
  // discord_id is the PK on NexusUser (BIGINT PK per schema) — required, not nullable.
  ...
  if (!discord_id) {
    return Response.json(
      { error: 'discord_id is required — it is the primary key on NexusUser' },
      { status: 400 }
    );
  }
  ...
  // NOTE: TOCTOU race condition — a concurrent request could pass this check
  // and then both insert. Mitigation requires a unique constraint on callsign
  // at the Base44 schema level; not addressable in this function alone.
...
async function handleReissue(base44, caller, body) {
  ...
  // Single atomic write — session_invalidated_at is a hard requirement.
  // If this update fails, the old key is not invalidated and the spec guarantee
  // is broken. Let the error propagate so the caller receives a 500, not silent success.
  await base44.asServiceRole.entities.NexusUser.update(target.id, {
    ...
    key_issued_by:          caller.discord_id,
    key_revoked:            false,
    roles_synced_at:        null, // Fix 5: force fresh Discord role sync on next login
    session_invalidated_at: now,  // Fix 3: hard requirement — must not be swallowed
  });
  ...
  base44.asServiceRole.functions
    .invoke('heraldBot', {
      ...
      payload: {
        event_type: 'REISSUED',
        callsign:   target.callsign,
        issued_by:  caller.discord_id,
        timestamp:  new Date().toISOString(),
      },
    })
    .catch(err => console.warn('[key-management] heraldBot keyEvent warning:', err.message));
...
async function handleRevoke(base44, caller, body) {
  ...
  // Single atomic write — session_invalidated_at is a hard requirement on revoke.
  // A revoked key with an active session is a security gap. Let failures propagate.
...
Deno.serve(async (req) => {
  try {
    ...
    console.error('[key-management] unhandled error:', err);
